...
class testQuantaxis(TestCase):
    ...
    def test_loop_QA_fetch_stock_day_adv(self):
        # 导入股票列表
        listings = qa.QA_fetch_stock_list_adv().code.tolist()
        df = pd.DataFrame()
        # codelist = []
    ...
    def test_QA_fetch_indexlisting(self):
        ...
        for i in df.index[:10]:
            ...
            data = qa.QAFetch.QATdx.QA_fetch_get_index_day(code, '2017-01-01', '2017-09-01')
            """
            d =  data.data.index[0][0].strftime("%Y-%m-%d")
            if a.sse == 'sh':
                market = 1
            else:
                market = 0
            category = 11
            querysetlist.append([("code", a.code), ("name", a['name']), ("timeToMarket", d), ("volunit", a.volunit), ("decimalpoint", a.decimal_point),
                        ("category", category), ("market", market)])
        self.assertTrue(len(querysetlist) > 0, '未插入成功:{}'.format(querysetlist))
        print("querysetlist\n{}".format(querysetlist))
